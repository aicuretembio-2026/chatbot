...
✅ 可以同時使用多種EXOBIO產品！`,
    ...
    {
      id: 'usage-04',
      keywords: ['MTS使用方法', 'MTS', '微針滾輪', '真皮滾輪'],
      category: '使用方法',
      question: 'MTS使用方法',
      answer: `🔬 **MTS（微針滾輪）使用方法**

📋 **使用方法**：
**01.** 使用前，將微針滾輪（MTS）浸泡在消毒用酒精中5～10分鐘，充分消毒。

**02.** 用潔面產品徹底清潔臉部後，調理肌膚。

**03.** 將EXOBIO外泌體精華充分塗抹於肌膚上。

**04.** 以均勻的壓力將微針滾輪輕柔地滾動在所需部位。重複滾動直到皮膚感受到輕微刺激。

**05.** 輕拍剩餘的精華使其吸收，紅腫的皮膚用舒緩霜完成護理。

⚠️ **使用須知**：
• MTS原則上為一次性產品，最多可使用3次。
• 再次使用時，務必每次徹底消毒後使用。
• 建議每3～4週使用1次。

💡 **效果**：與外泌體精華一起使用時，皮膚吸收率提高，效果更佳！`,
      relatedQuestions: ['使用方法', '混合使用'],
...
💡 **訣竅**：搭配EXOSOME 7（每週3-4次）+ RED 7（每日）以獲得更快結果！`,
  ...
  quickReplyKeywords: [
    ...
    { label: '💫 皺紋', query: '皺紋' },
    { label: '💫 細紋', query: '細紋' },
    { label: '💫 深層皺紋', query: '深層皺紋' },
    { label: '💫 眼周皺紋', query: '眼周皺紋' },
    { label: '💫 法令紋', query: '法令紋' },
    
    // 肌膚問題 - 美白/色素 (4)
    { label: '💎 美白', query: '美白' },
    { label: '✨ 黑斑', query: '黑斑' },
    { label: '💎 色素沉澱', query: '色素沉澱' },
    { label: '💎 暗啞', query: '暗啞肌膚' },
    
    // 肌膚問題 - 痘痘/問題 (5)
    { label: '🔴 痘痘', query: '痘痘' },
    { label: '🔴 青春痘', query: '青春痘' },
    { label: '🔴 問題肌膚', query: '問題肌膚' },
    { label: '🔴 痘印', query: '痘印' },
    { label: '⭕ 毛孔', query: '毛孔' },
    ...
    { label: '🌿 舒緩', query: '舒緩' },
    { label: '🌸 敏感', query: '敏感肌膚' },
    { label: '🛡️ 屏障', query: '肌膚屏障' },
    { label: '🛡️ 屏障護理', query: '強化屏障' },
    ...
    // 使用方法 (4)
    { label: '📖 使用方法', query: '使用方法' },
    { label: '📖 用量', query: '用量' },
    { label: '📖 搭配使用', query: '搭配使用' },
    { label: '🔬 MTS使用方法', query: 'MTS使用方法' },
    
    // 產品資訊 (7)
    { label: '🎯 產品推薦', query: '推薦' },
    { label: '🔄 差異', query: '差異' },
    { label: '💡 關於EXOBIO', query: 'EXOBIO' },
    { label: '✅ 功效', query: '功效' },
    { label: '🔢 數字意義', query: '357' },
    { label: '💰 價格', query: '價格' },
    { label: '🧪 成分', query: '成分' },
